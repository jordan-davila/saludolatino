<template>
    <div class="relative flex-1">
        <div class="search flex flex-1 justify-center items-center text-white font-normal h-full relative border-r border-solid border-gray-700">
            <SearchFocus @keyup="focusSearch"></SearchFocus>
            <i class="far fa-search absolute top-0 left-0 mt-5 ml-8"></i>
            <input
                v-model="query"
                @blur="visible = false"
                @focus="visible = true"
                ...
                ref="search"
                type="text"
                class="bg-transparent h-full w-full px-16 text-sm focus:outline-none placeholder-gray-700 focus:border-indigo-600 focus:border-solid focus:border-b-2"
                placeholder='Buscar Estrella  ( Presiona " / " Para Enfocar )'
            />
        </div>
        <div
